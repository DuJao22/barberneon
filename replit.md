...
- **Real-time Notifications**: Implemented for new appointments on the admin dashboard using a polling mechanism to `/api/admin/check-novos-agendamentos`. Utilizes browser Notification API for detailed alerts, local storage for ID persistence, and automatic page reload.
